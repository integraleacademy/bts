...
@app.route("/logout")
def logout(): session.pop("is_admin",None); return redirect(url_for("login"))
...
def update(id):
    ...
    data=_load_data()
    for r in data:
        if r["id"]==id: r["status"]=st; break
    _save_data(data); return redirect(url_for("admin"))

@app.route("/delete/<id>",methods=["POST"])
@require_admin
def delete(id):
    data=_load_data(); new=[r for r in data if r["id"]!=id]; _save_data(new)
